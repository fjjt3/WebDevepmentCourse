...
console.log(myArr[0]);
console.log(myArr.at(0)); //  let negatives number
...
const myMovements = [200, 450, -400, 3000, -650, -130, 70, 1300];

// for (const movement of movements) {
for (const [i, movement] of myMovements.entries()) {
  if (movement > 0) {
...
console.log("---- FOREACH ----");
myMovements.forEach(function (mov, i, arr) {
  if (mov > 0) {
...
const checkDogs = function (dogsJulia, dogsKate) {
  let dogJuliaCorrected = dogsJulia.slice();
  dogJuliaCorrected = dogJuliaCorrected.splice(0);
  dogJuliaCorrected = dogJuliaCorrected.splice(-2);
...
const ownersEatTooMuch = dogs
  .filter(dog => dogs.curFood > dogs.recFood)
